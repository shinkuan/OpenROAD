...
int DRVExtractor::run(const std::string& inputODBFile, const std::string& outputDir) {
    logger.info("Running DRVExtractor with ODB file: %s, output dir: %s",
                inputODBFile.c_str(), outputDir.c_str());
    // Load the database
    ...
    odb::dbSet<odb::dbMarkerCategory> marker_categories = block->getMarkerCategories();
    logger.info("Total marker categories in block: %zu", marker_categories.size());
    for (auto category : marker_categories) {
        std::ofstream json_file(outputDir + "/" + category->getName() + ".json");
        category->writeJSON(json_file);
        json_file.close();
    }
    return 0;
}

int DRVExtractor::loadDatabaseFromFile_(const std::string& filename, odb::dbDatabase** db_ptr) {
